...
const long dlgConnectionManager::ID_CONTROLDEVICE_RGR = wxNewId();
...
void dlgConnectionManager::SetConnectionManager(ConnectionManager *pManager)
{
    m_pManager = pManager;
}
...
//    if( selTransmitter >= 0)
//      m_pManager->OpenTransmitter( selTransmitter );
...
void dlgConnectionManager::OnInit(wxInitDialogEvent& event)
{
    ...
    listReceivers->SetSelection( m_pManager->GetOpenedReceiver());
