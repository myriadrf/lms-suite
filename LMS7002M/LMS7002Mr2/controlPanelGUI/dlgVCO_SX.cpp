...
dlgVCO_SX::dlgVCO_SX( PLL_SX *data, wxWindow* parent,wxWindowID id,const wxPoint& pos,const wxSize& size)
{
    pll_data = data;
...
void dlgVCO_SX::OnInit(wxInitDialogEvent& event)
{
    ...
    pnlVCOH->Initialize("VCOH", freqs);

    freqs = pll_data->GetVCOMFrequencies();
    for(unsigned i=0; i<freqs.size(); ++i)
        freqs[i] = freqs[i]/1000; // convert from MHz to GHz
    pnlVCOM->Initialize("VCOM", freqs);

    freqs = pll_data->GetVCOLFrequencies();
    for(unsigned i=0; i<freqs.size(); ++i)
        freqs[i] = freqs[i]/1000; // convert from MHz to GHz
    pnlVCOL->Initialize("VCOL", freqs);
}
