...
// DESCRIPTION:	main class for controlling LMS7002 chip
// DATE:		2013-11-28
...
*/
LMS7002_MainControl::LMS7002_MainControl() : m_device(new ConnectionManager())
{
    algorithms = new Algorithms(m_device);
...
bool LMS7002_MainControl::LoadFromFile(const string filename)
{
    ...
    if(!loadFromBinary)
    {
        ...
        if(fileVersion == 1)
        {
            ...
            m_freqCGEN_MHz = parser.Get("CGEN frequency MHz", 600);
...
Si5351C* LMS7002_MainControl::getSi5356() const
{
    return m_si5356a;
}
...
bool LMS7002_MainControl::LoadGFIRCoefficients(short *coefficients, int coef_count, int id, bool Rx)
{
    ...
    if(coef_count>200)
        coef_count = 200;
    memcpy(cbuf, coefficients, sizeof(unsigned short)*coef_count);
    int iVal;
    iVal = coef_count;
...
*/
int LMS7002_MainControl::SetFrequency(const double desFreq_MHz, double &realFreq_MHz, unsigned &Nint, unsigned &NFrac, unsigned &iVCO, double &VCO_Freq_MHz, unsigned &iDind, unsigned &iEnDiv2, int EnDiv2mode)
{
    double refClk_MHz;
...
bool LMS7002_MainControl::SetNCOconfiguration(const vector<double> &FCW, const vector<int> &PHO, const bool Rx)
{
    ...
    for(unsigned int i=0; i<16 && i<FCW.size(); ++i) //create FCW codes from frequencies
    {
        fcwCodes.push_back( CalculateNCO_FCW_Code(FCW[i], refClk) );
    }
    ...
    if( GetParam(mode) == 0) //Write FCW
    {
        if(fcwCodes.size() < 16)
        {
            ss << "Configuring NCO " << (Rx ? "Rx" : "Tx") << ": FCW codes given " << fcwCodes.size() << ", needed 16." << endl;
            MessageLog::getInstance()->write(ss.str(), LOG_WARNING);
        }
        ss.str( string() );
        ss << "Configuring NCO " << (Rx ? "Rx" : "Tx") << ": FCW codes" << endl;
        addr = 0x0242;
        if(Rx)
        {
            addr += 0x0200;
    ...
    else //Write PHO
    {
        if(PHO.size() < 16)
        {
            ss << "Configuring NCO " << (Rx ? "Rx" : "Tx") << ": PHO codes given " << PHO.size() << ", needed 16." << endl;
            MessageLog::getInstance()->write(ss.str(), LOG_WARNING);
        }
        ss.str( string() );
    ...
    if( status == STATUS_COMPLETED_CMD)
    {
        MessageLog::getInstance()->write("NCO configured\n", LOG_INFO);
        return true;
    }
    else
    {
